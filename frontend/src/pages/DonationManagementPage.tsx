      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col span={24}>
          <Alert
            ...
            description={
              <div>
                <p><strong>{t("賣家收益：")}</strong>{t("非線上交易商品的賣家實際收益（價格 - 捐贈金額）")}</p>
                <p><strong>{t("須追繳金額：")}</strong>{t("線上交易商品的捐贈金額（賣家直接收取全額，需向賣家追繳捐贈額）")}</p>
                <p><strong>{t("線上交易：")}</strong>{t("賣家與買家直接交易，賣家收取全額但需向管理員繳納捐贈額")}</p>
                <p><strong>{t("非線上交易：")}</strong>{t("管理員處理金流，捐贈額已從賣家收益中扣除")}</p>
              </div>
            }
            type="info"
            showIcon
